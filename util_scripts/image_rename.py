...
def generate_filename_with_timestamp(file_path):
    ...
    if not new_file_name:
        logging.error(
            "Couldn't find file creation time for - {}".format(file_path))
...
def rename_images(image_folder_path, destination_dir=None, move_to_dest=False):
    file_name_dict = dict()
    ...
    for walk_dir, dirs, files in os.walk(image_folder_path):
        for file in files:
            ...
            if not file_extension in image_extensions:
                logging.info(
                    "File {} is not an image. Skipping process".format(f))
                continue
            ...
            if file_extension == ".CR2":
                logging.info("Found CR2 image - {}{}".format(
                    f, file_extension))
                if "CR2" in walk_dir:
                    logging.info(
                        "Skipping creating another CR2 in {}".format(dest_dir))
                    continue
            ...
            new_file_path = os.path.join(
                dest_dir, new_file_name + file_extension.lower())
            logging.info("Renaming {0} to {1}{2}".format(
                file_path, new_file_name, file_extension))

            if move_to_dest:
                try:
                    shutil.move(file_path, new_file_path)
                except Exception as e:
                    logging.error("Couldn't perform " + file_path + "->" +
                                  new_file_name)
                    logging.error(e)
                    skipped_file_count += 1
            else:
...
def main():
    ...
    parser.add_argument(
        '-v', '--verbose', default=False, help='logging.infos logs in detail')
    ...
    # Processing files
    processed_files, skipped_files = rename_images(path, args.output,
                                                   args.move)
